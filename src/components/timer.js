...
function MyTimer({ roundDuration, nextRound }) {
  ...
  const {
    ...
    resume,
    restart,
  } = useTimer({ expiryTimestamp, onExpire: () => handleExpire(), autoStart: false });
  ...
  return (
    <div className='flex flex-col w-full bg-primary text-black items-center text-center justify-center rounded-lg shadow-lg gap-5 p-5'>
        ...
        <section className='flex flex-row gap-2'>
            <button className='p-2 bg-gray-800/15 rounded-lg shadow-sm' onClick={resume}>Play</button>
            <button className='p-2 bg-gray-800/15 rounded-lg shadow-sm' onClick={pause}>Pause</button>
            <button className='p-2 bg-gray-800/15 rounded-lg shadow-sm' onClick={handleRestart}>Restart</button>
        </section>
    </div>
  );
}
